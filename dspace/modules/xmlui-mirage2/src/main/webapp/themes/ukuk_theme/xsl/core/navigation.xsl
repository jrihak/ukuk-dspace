...
<xsl:stylesheet xmlns:i18n="http://apache.org/cocoon/i18n/2.1"
    ...
    <xsl:template match="dri:options">
        <div id="ds-options" class="word-break hidden-print">
            <xsl:if test="not(contains(/dri:document/dri:meta/dri:pageMeta/dri:metadata[@element='request'][@qualifier='URI'], 'discover'))">
                <!-- <JR> - 22. 2. 2017 - odstraněn vyhledávací formulář z postranního panelu DSpace -->
        ...
        <!-- <AM> 13.6.2017 - přidáno rozbalovací menu s užitečnými odkazy -->
            <div>
                <h2 class="ds-option-set-head h6">
                <i18n:text>xmlui.ArtifactToplinks.Navigationhead.all_of_dspace_mainlink</i18n:text>
                </h2>
            </div>
    ...
    <xsl:template match="dri:options//dri:list">
        ...
        <!-- Přidá div ke každé z kategorií v postranním panelu-->
        <xsl:element name="div">
            <xsl:attribute name="id">
                <xsl:value-of select="translate(./node()[1], '. ', '')"/>
            </xsl:attribute>
            <xsl:attribute name="class">
                <xsl:text>panel-collapse collapse out panel-body sidebar-panel-body</xsl:text>
            </xsl:attribute>
            <xsl:attribute name="aria-labelledby">
                <xsl:text>tab-</xsl:text><xsl:value-of select="translate(./node()[1], '. ', '')"/>
            </xsl:attribute>
    ...
    <xsl:template match="dri:options/dri:list//dri:list/dri:head" priority="3">
        <xsl:element name="div">
            ...
            <xsl:attribute name="id">
                <xsl:text>tab-</xsl:text><xsl:value-of select="translate(., '. ', '')"/>
            </xsl:attribute>
            <xsl:attribute name="class">
                <xsl:text>panel-heading sidebar-panel-heading</xsl:text>
            </xsl:attribute>
            <xsl:element name="a">
                <xsl:attribute name="class">
                ...
                <!--<a class="list-group-item active" role="button" data-toggle="collapse" href="#collapseExample1" aria-expanded="false" aria-controls="collapseExample1">-->
                    <!--<span>-->
                        <!--<xsl:call-template name="standardAttributes">-->
                            <!--<xsl:with-param name="class">-->
                                <!--<xsl:value-of select="@rend"/>-->
                    ...
                    <!--</span>-->
                <!--</a>-->
        <!--<a class="list-group-item active" role="button" data-toggle="collapse" href="#collapseExample" aria-expanded="false" aria-controls="collapseExample">-->
            <!--<span>-->
                <!--<xsl:call-template name="standardAttributes">-->
                    <!--<xsl:with-param name="class">-->
                        <!--<xsl:value-of select="@rend"/>-->
